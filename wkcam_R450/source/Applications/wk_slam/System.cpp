        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    mptLocalMapping = new thread(&WK_SLAM::LocalMapping::Run,mpLocalMapper);

    //Initialize the Loop Closing thread and launch
    mpLoopCloser = new LoopClosing(mpMap, mpKeyFrameDatabase, mpVocabulary, mSensor!=MONOCULAR);
    mptLoopClosing = new thread(&WK_SLAM::LoopClosing::Run, mpLoopCloser);
...
void System::TrackMonocular(wk_corner_video_frame_s::wk_ptr img_data)
{
    cv::Mat im;
    ...
    if(mTrackingState == Tracking::eTrackingState::OK)
    {
        cv::Mat Tlc = Tlw * Tcw.inv();
        ...
        result->corner_num = 50;

        wk_st_lk_middle::wk_st_lk_get_instance()->wk_result_export(result);

        Tlw = Tcw;

        std::cout << "TrackMonocular Tcw:" << std::endl << Tcw << std::endl;
    }
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    list<WK_SLAM::KeyFrame*>::iterator lRit = mpTracker->mlpReferences.begin();
