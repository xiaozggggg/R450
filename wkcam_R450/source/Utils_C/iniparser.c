...
//================ walkera add for set ini value =================//
static int EXpnewline(char c)
{
    return ('\n' == c ||  '\r' == c )? 1 : 0;
}
static int EXpend_of_string(char c)
{
    return '\0'==c? 1 : 0;
}
static int EXpleft_barce(char c)
{
    return LEFT_BRACE == c? 1 : 0;
}
static int EXpisright_brace(char c )
{
                      ...
                      int *key_s,int *key_e, int *value_s, int *value_e)
{
    ...
    *sec_e = *sec_s = *key_e = *key_s = *value_s = *value_e = -1;
    ...
    while( !EXpend_of_string(p[i]) ) {
        //find the section
        if( ( 0==i ||  EXpnewline(p[i-1]) ) && EXpleft_barce(p[i]) )
        {
            ...
            do {
                i++;
            } while( !EXpisright_brace(p[i]) && !EXpend_of_string(p[i]));
            ...
            if( 0 == strncmp(p+section_start,section, i-section_start)) {
                ...
                *sec_e = i;
                ...
                while( ! (EXpnewline(p[i-1]) && EXpleft_barce(p[i]))
                && !EXpend_of_string(p[i]) ) {
                    int j=0;
                    //get a new line
                    newline_start = i;

                    while( !EXpnewline(p[i]) &&  !EXpend_of_string(p[i]) ) {
                        i++;
                    }
...
int load_ini_file(const char *file, char *buf,int *file_size)
{
    ...
    *file_size =0;
    ...
    while( buf[i]!= (char)EOF) {
        i++;
        assert( i < MAX_FILE_SIZE ); //file too big, you can redefile MAX_FILE_SIZE to fit the big file
        buf[i]=fgetc(in);
    }
