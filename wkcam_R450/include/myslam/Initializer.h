...
public:
    ...
    Initializer(const Frame &ReferenceFrame, const cv::Mat& K, float sigma = 1.0, int iterations = 200);
...
private:
    ...
    void Normalize(const std::vector<std::shared_ptr<Feature>> &vKeys, std::vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T);

    int CheckRT(const cv::Mat &R, const cv::Mat &t, const std::vector<std::shared_ptr<Feature>> &vKeys1, const std::vector<std::shared_ptr<Feature>> &vKeys2,
    ...
    std::vector<std::shared_ptr<Feature>> mvKeys1;

    // Keypoints from Current Frame (Frame 2)
    std::vector<std::shared_ptr<Feature>> mvKeys2;
