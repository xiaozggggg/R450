...
void wk_log_init()
{
	logger_t* log_info = hv_default_logger();

	/* 设定打印等级和颜色 */
	logger_set_level(log_info, LOG_LEVEL_DEBUG);
	logger_enable_color(log_info, 1);
	logger_set_format(log_info, NULL);

	/* 保存日志在本地文件，期限1天 */
	logger_set_file(log_info, "./debug/wk_log");
	logger_set_max_filesize(log_info, 1024*1024);
	logger_enable_fsync(log_info, 1);
	logger_set_remain_days(log_info, 1);

	logger_set_handler(log_info, wk_log_handle);
...
#undef XXX
    ...
    len += snprintf(buf + len, bufsize - len, "%04d-%02d-%02d %02d:%02d:%02d:%03d %s [%s:%s:%d] ",
        year, month, day, hour, min, sec, us/1000,
        plevel, file, function, line);
