...
typedef enum {
    ...
    OT_DIS_MODE_GYRO_ADVANCE,        /* Only use with gyro advance. */
    OT_DIS_MODE_BUTT,
} ot_dis_mode;
...
typedef enum {
    OT_DIS_PDT_TYPE_RECORDER = 0,   /* IPC product type. */
...
typedef struct {
    td_s32 steady_atten_coef;   // Range:[0, 100]
    td_s32 motion_atten_coef;   // Range:[0, 1000]
} ot_atten_coef;

typedef struct {
    ot_atten_coef atten_coef;
} ot_gyrodis_alg_attr;

typedef struct {
    ot_gyrodis_alg_attr gyrodis_alg_attr;
} ot_dis_alg_attr;
